...
% Pied de page : URL du dépôt à gauche, numéro de page à droite
\renewcommand{\ps@plain}{%
  \let\@mkboth\@gobbletwo
  \let\@oddhead\@empty
  \def\@oddfoot{\reset@font\footnotesize\href{https://github.com/micmia/AI-Fairness-Jigsaw}{\texttt{github.com/micmia/AI-Fairness-Jigsaw}}\hfil\thepage}%
  \let\@evenhead\@empty
...
\title{%
  ...
  {\normalsize Rapport — Équité algorithmique, interprétabilité et robustesse}\\[0.4em]
...
Classification binaire toxique / non toxique. Nous comparons une \textbf{ToxicityGRU} (baseline) à trois variantes (répondération, adversarial, combiné), avec métriques d'équité sur 18 sous-groupes, \textsc{SHAP} mot à mot, et robustesse sous bruit caractère, dropout de mots et troncature. Les champs d'identité binaires suivent le schéma annoté Jigsaw ; l'EDA met en avant les groupes au biais d'étiquetage le plus marqué. Pipeline reproductible dans le notebook \path{notebooks/10-final.ipynb} (EDA $\to$ entraînement $\to$ évaluation d'équité $\to$ SHAP $\to$ robustesse).

\section{Protocole}

\paragraph{Données.}
\texttt{id}, \texttt{target}, \texttt{comment\_text} et indicateurs d'identité depuis \path{data/train.csv} ; \texttt{toxic} dérivé de \texttt{target} au seuil 0,5. Toute l'évaluation d'équité porte sur le sous-ensemble annoté (commentaires dont les champs d'identité sont renseignés, $\approx 25\,\%$ du corpus, sur-échantillonné en toxicité). Split 80/20 stratifié (\texttt{random\_state=42}), sauvegardé dans \path{data/split_ids.json} pour figer la répartition entre exécutions.

\paragraph{Modèle et encodage.}
Tokenizer \texttt{bert-base-uncased}, $L=128$. \textbf{ToxicityGRU} (baseline, répondération) ; \textbf{FairGRU} + GRL (Gradient Reversal) pour adversarial et combiné.
...
\paragraph{Adversarial + GRL (Gradient Reversal Layer).} FairGRU : tête toxicité + discriminateur d'identité ; le GRL inverse le gradient vers l'encodeur pour appauvrir la représentation en signal d'identité.

\paragraph{Combiné.} Répondération + objectif adversarial.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{2.55cm}cccccccc}
    ...
    \bottomrule
  \end{tabular}
  \caption{Détail par identité : AUC BPSN (colonnes 2--5) et écart FPR (colonnes 6--9 ; proche de 0 favorable). Gras = meilleur dans chaque bloc de quatre valeurs. \texttt{christian} sert de groupe de contrôle (faible biais d'étiquetage en EDA), attendu stable d'une mitigation à l'autre.}
  \label{tab:detail_id}
\end{table}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{llcccc}
    ...
    \bottomrule
  \end{tabular}
  \caption{Robustesse : AUC sur sous-échantillon de test perturbé. Bruit caractère (probabilité de substitution par caractère), dropout aléatoire de mots, troncature de la séquence ($L$ tokens). Gras = meilleur par ligne.}
